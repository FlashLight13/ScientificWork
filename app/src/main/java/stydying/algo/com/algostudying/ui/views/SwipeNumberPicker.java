...
public class SwipeNumberPicker extends TextView {
    ...
    private void init(Context context, AttributeSet attributeSet) {
        ...
        this.mOnValueChangeListener = new OnValueChangeListener() {
            @Override
    ...
    private void customize() {
        left = getDrawableCompat(R.drawable.ic_arrow_left);
        right = getDrawableCompat(R.drawable.ic_arrow_right);
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        setMinHeight((int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_PX,
                arrow.getBounds().height() * 1.5f,
                getContext().getResources().getDisplayMetrics()));
    }
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                if (Math.abs(currentX - mStartX) > mGestureStepPx) {
                    ...
                    double swipedDistance = TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_PX,
                            Math.abs(distance), getContext().getResources().getDisplayMetrics());
                    float threshold;
    ...
    private void processClick(MotionEvent event) {
        ...
        final int leftDrawableLeft = 0;
        final int leftDrawableRight = leftDrawableLeft + left.getIntrinsicWidth();
        if (clickX > leftDrawableLeft && clickX < leftDrawableRight) {
            changeValue((int) mIntermediateValue - 1);
            notifyListener((int) mIntermediateValue - 1);
            return;
        }
        final int rightDrawableRight = getWidth();
        final int rightDrawableLeft = rightDrawableRight - right.getIntrinsicWidth();
        if (clickX > rightDrawableLeft && clickX < rightDrawableRight) {
            changeValue((int) mIntermediateValue + 1);
            notifyListener((int) mIntermediateValue + 1);
            return;
        }
    ...
    private Drawable createBackgroundDrawable(int color) {
        ShapeDrawable backgroundDrawable = new ShapeDrawable(
                new RoundRectShape(new float[]{mCornerRadius,
                        mCornerRadius, mCornerRadius,
                        mCornerRadius, mCornerRadius, mCornerRadius,
                        mCornerRadius, mCornerRadius}, null, null));
